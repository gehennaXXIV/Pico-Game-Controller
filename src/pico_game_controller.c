...
/**
 * Keyboard Mode (v1.2 Cleaned - No Mouse/Encoders)
 **/
void key_mode() {
  if (tud_hid_ready()) {
    /*------------- Keyboard -------------*/
    ...
    for (int i = 0; i < SW_GPIO_SIZE; i++) {
      // Check if button is pressed in our report bitmask
    ...
    // We no longer toggle kbm_report or send mouse reports
  }
}
