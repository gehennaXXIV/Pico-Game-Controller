...
static inline void handle_brightness_shortcuts() {
    if (!gpio_get(SW_GPIO[9])) { // Hold Button 10 (GPIO 9)
        static uint32_t last_tick = 0;
        uint32_t now = to_ms_since_boot(get_absolute_time());

        if (now - last_tick > 50) {
            // MODE SWITCHING
            if (!gpio_get(SW_GPIO[0])) light_mode = 1; // Button 1 -> Breathing
            if (!gpio_get(SW_GPIO[1])) light_mode = 0; // Button 2 -> Ping-Pong
            if (!gpio_get(SW_GPIO[2])) light_mode = 2; // Button 3 -> Droplets (Index 2)
